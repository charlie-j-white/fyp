...
C   RW status of diff variables: j:out flow:in arearat:in
      SUBROUTINE COST_D(j, jd, arearat, arearatd, flow, flowd, ni)
      IMPLICIT NONE
C
C
C
C
      ...
      DOUBLE PRECISION, DIMENSION(1, 3*ni+9), INTENT(IN) :: flow
      DOUBLE PRECISION, DIMENSION(1, 3*ni+9), INTENT(IN) :: flowd
C
      INTEGER ii, i
      ...
      DOUBLE PRECISION urd(-1:ni+1), umd(-1:ni+1), ued(-1:ni+1)
      INTRINSIC DSQRT
...
C
      ...
      ENDDO
C
C     assign variables from concatenation
C
      DO i=1,ni+3
        urd(i-2) = flowd(1, i)
        ur(i-2) = flow(1, i)
