...
C
C      call pressure(nx,ny,nl,pres,u1,u2,u3,u5)
C
      DO i=1,4
        da(i) = 0.0d0
        db(i) = 0.0d0
...
C
C      print*
C      print*, xa,xb,xc,xd,ya,yb,yc,yd
C      print*, 
C     & 180.0d0*DATAN2(-xa,ya)/pi,
C     & 180.0d0*DATAN2(-xb,yb)/pi,
C     & 180.0d0*DATAN2(-xc,yc)/pi,
C     & 180.0d0*DATAN2(-xd,yd)/pi
C
C
C
C
...
C
C
C      print*, r1(i,j), (da(1) + db(1) - dc(1) - dd(1))
C      print*, r2(i,j), (da(2) + db(2) - dc(2) - dd(2))
C      print*, r3(i,j), (da(3) + db(3) - dc(3) - dd(3))
C      print*, r5(i,j), (da(4) + db(4) - dc(4) - dd(4))
C
C
C
C
...
C
C
C     final closing bits
C-----------------------------------------------------------------------
C
C     write info to file
C
C      open(100, file='mesh.plt')
C      write(100,*) 'VARIABLES = "x" "y"'
C      write(100,*) 'Zone I=',ny+1,', J=',nx+1,', F=POINT'
C101   format(2f14.8)
C!
C      do i = 0,nx
C      do j = 0,ny
C            write(100,101) meshX(i,j), meshY(i,j)
C      end do
C      end do
C!
C      close(100)
...
C!
C!
C      open(200, file='transect.plt')
C      write(200,*) 'VARIABLES = "x" "y"'
C201   format(2f14.8)
C!
C      do i = 1,nx+1
C            write(200,201) xt(i), yt(i)
C      end do
C!
C      close(200)
C
...
C     do the actual spline formula
 33     ytd(i) = ad(j) + (xt(i)-xc(j))**1.0d0*bd(j) + (xt(i)-xc(j))**
...
C
        DO i=1,nx
          phi_igd(i) = ((phi_igd(i)-phi_igd(1))*(phi_ig(nx)-phi_ig(1))-(
     +      phi_ig(i)-phi_ig(1))*(phi_igd(nx)-phi_igd(1)))/(phi_ig(nx)-
     +      phi_ig(1))**2
...
C
C     do the actual spline formula
 55       ytd(i) = ad(j) + bd(j)*(xt(i)-xc(j))**1.0d0 + b(j)*xtd(i) + cd
...
C      print*, flow
C!
C      call debug_cell(nx,ny,nl,u1)
C      call debug_cell(nx,ny,nl,u2)
C      call debug_cell(nx,ny,nl,u3)
C      call debug_cell(nx,ny,nl,u5)
C!
C      print*, "--------------------------------------------------------"
C      print*, "     "
C
...
C
C
C     seed flow vector with test values
C
C      do i = 1,4*nh+4*nb
C      flow(1,i) = i
C      end do
